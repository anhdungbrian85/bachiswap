...
const check_TX_queue = async () => {
  if (global_vars.is_check_TX_queue_all) return;
  global_vars.is_check_TX_queue_all = true;
  try {
    console.log(
      `${
      ...
      } TXs in check_TX_queue_all at ${convertToUTCTime(new Date())}`
    );

    let queue_data = await TransactionQueue.find({
    ...
    console.log(
      `${
        CONFIG_TYPE_NAME.BC_PROCESSING_ALL_QUEUE_TX
      } - Start processing ${records_length} TXs in check_TX_queue_all at ${convertToUTCTime(
        new Date()
      )}`
    );
    ...
    console.log("check_TX_queue_all - " + error.message);
    global_vars.is_check_TX_queue_all = false;
  }
  global_vars.is_check_TX_queue_all = false;
};
